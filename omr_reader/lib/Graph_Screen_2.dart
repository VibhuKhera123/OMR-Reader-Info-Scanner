...
class Stud_data {
  ...
  Stud_data(
      {this.id, this.marks, this.Score_perc, this.mistake_data, this.devi_scr});
  ...
  static List<Stud_data> getData() {
    return <Stud_data>[
      ...
      for (var i = 0; i < analysis_data['Enroll_id'].length; i++)
        Stud_data(
            id: analysis_data['Enroll_id'][i],
            marks: analysis_data['Marks'][i],
            Score_perc: analysis_data['Percentage'][i],
            mistake_data: analysis_data['Ned_Ques'][i],
            devi_scr: analysis_data['DFA'][i]),
    ];
  }
}
          ...
          .map(
            (user) => DataRow(
              cells: [
                ...
                // Cell 1
                DataCell(Text(user.Score_perc.toString())),

                // Cell 1
                DataCell(Text(user.mistake_data.toString())),

                // Cell 1
                DataCell(Text(user.devi_scr.toString())),
              ],
            ),
